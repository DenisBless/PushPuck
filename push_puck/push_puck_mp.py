...
class PushPuckMultiProcessing:
    def __init__(self, push_puck, n_workers) -> None:
    ...
    def work(self, weights, goal_pos, pid, reward_arr, lock, n_samples_per_worker):
        ...
        with lock:
            pp = self.push_puck()  # Create instance of PushPuck obj
...
if __name__ == '__main__':
    pp_mp = PushPuckMultiProcessing(push_puck=PushPuck7DoF, n_workers=6)
    rewards = pp_mp(weights=np.arange(32), goal_pos=np.zeros([32, 1]))
    print(rewards)
